...
# from multigpu_train import gpus_count
import tensorflow as tf

from data_util import GeneratorEnqueuer
import config

tf.app.flags.DEFINE_string('training_data_path',config.training_data_path ,
...
tf.app.flags.DEFINE_string('geometry', config.geometry,
                           'which geometry to generate, RBOX or QUAD')


FLAGS = tf.app.flags.FLAGS

gpus = list(range(FLAGS.gpu_list))
...
def get_new_batch(images, image_fns, score_maps, geo_maps, training_masks,batch_size=FLAGS.batch_size_per_gpu * len(gpus)):
    ims = [] ; im_fns = [] ; s_maps = [] ; g_maps =[] ; t_masks = []
    for _ in range(batch_size):
        idx = np.random.randint(1,len(images))
        ims.append(images[idx].astype(np.float32))
        im_fns.append(image_fns[idx])
        s_maps.append(score_maps[idx])
        g_maps.append(geo_maps[idx])
        t_masks.append(training_masks[idx])
    # print("&&&&&&&&&&&&&&&&&&&&&&&&",(np.array(g_maps[0])).shape,(np.array(t_masks[0])).shape)
    # print(len(ims),len(im_fns),len(s_maps),len(g_maps),len(t_masks))
    return (ims,im_fns,s_maps,g_maps,t_masks)
